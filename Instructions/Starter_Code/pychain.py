...
sender_address = st.text_input("Enter Sender address")
...
if st.button("Add Block"):
    ...
    new_block = Block(
        record = Record(sender_address,receiver_address, amount),
        creator_id = 42,
...
# Test the complete `PyChain` ledger and user interface by running the Streamlit application and storing some mined blocks in your `PyChain` ledger. Then test the blockchain validation process by using the `PyChain` ledger.
# 1. In the terminal, navigate to the project folder where the challenge has been saved.
